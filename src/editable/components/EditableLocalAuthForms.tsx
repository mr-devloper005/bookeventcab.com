...
const inputClass = 'h-12 w-full rounded-2xl border border-[var(--editable-border)] bg-[var(--slot4-page-bg)] px-4 text-sm font-medium text-[var(--slot4-page-text)] outline-none transition placeholder:text-[var(--slot4-soft-muted-text)] focus:border-[var(--slot4-accent)]'
const buttonClass = 'inline-flex h-12 w-full items-center justify-center rounded-full bg-[var(--slot4-accent)] px-6 text-sm font-semibold text-[var(--slot4-on-accent)] transition hover:-translate-y-0.5 hover:shadow-[0_14px_34px_rgba(25,23,17,0.26)] active:translate-y-0 disabled:cursor-not-allowed disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-2xl px-4 py-3 text-sm font-semibold ${status === 'success' ? 'border border-emerald-200 bg-emerald-50 text-emerald-700' : 'border border-[var(--editable-border)] bg-[var(--slot4-accent-soft)] text-[var(--slot4-page-text)]'}`}>{message}</p> : null}
